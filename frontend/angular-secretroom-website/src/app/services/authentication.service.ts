...
export class AuthenticationService {
  ...
  registration(user: User): Observable<UserDetails> {
    const url = environment.shopApiUrl +'v1/users';
  ...
  login(user: User): Observable<UserDetails> {
    const url = environment.shopApiUrl + 'v1/users/login';
    return this.http.post<UserDetails>(url, user).pipe(
      tap(() => {
        this.loggedIn.next(true);
      })
  ...
  logout(): void {
    ...
    if (userDetails) {
      ...
      if (token && refreshToken) {
        this.http.post(environment.shopApiUrl + 'v1/users/logout', {
  ...
  restorePassword(email: string, lang: string): Observable<any> {
    const url = environment.shopApiUrl +  'v1/users/restore-password'; // Replace with your restore password API endpoint from UserController
    return this.http.get(url, { params: { email, lang } });
  }

  resetPassword(token: string, newPassword: string): Observable<void> {
    const url = environment.shopApiUrl +  'v1/users/reset-password'; // Replace with your reset password API endpoint from UserController
